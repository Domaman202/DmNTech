...
public abstract class MachineCasingBE extends MachineBE {
    ...
    public MachineCasingBE(BlockEntityType<?> type, BlockPos pos, BlockState state, int internal, int io) {
        this(type, pos, state, new SimpleConfigurableInventory(1), internal, io);
    }

    public MachineCasingBE(BlockEntityType<?> type, BlockPos pos, BlockState state, ConfigurableInventory inventory, int internal, int io) {
        this(type, pos, state, inventory, 0, 0, internal, io);
    }

    public MachineCasingBE(BlockEntityType<?> type, BlockPos pos, BlockState state, long energy, long maxEnergy, int internal, int io) {
        this(type, pos, state, new SimpleConfigurableInventory(1), energy, maxEnergy, internal, io);
    }

    public MachineCasingBE(BlockEntityType<?> type, BlockPos pos, BlockState state, ConfigurableInventory inventory, long energy, long maxEnergy, int internal, int io) {
    ...
    public class UnsafeProperties implements PropertyDelegate {
        ...
        public void set(int index, int value) {
            switch (index) {
                ...
                case 3 -> Machine.setActive(value == 1, getWorld(), getPos());
            }
        }
